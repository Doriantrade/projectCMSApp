using CMS_System.Controllers.TransferenciasProd;
...
namespace CMS_System.Controllers
{
    ...
    public class AsignacionMaquinariaAgenciaController : ControllerBase
    {
        ...
        public AsignacionMaquinariaAgenciaController( cmsDb2024Context context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> GuardarMaquinaAgencia([FromBody] Asignacionmaquinacliente model)
        {
            ...
            {
                _context.Asignacionmaquinacliente.Add(model);
        ...
        //{
        //    var resultado = await (from asm in _context.Asignacionmaquinacliente
        //                           join mq in _context.Maquinaria on asm.Codprod equals mq.Codmaquina into mqJoin
        //                           from mq in mqJoin.DefaultIfEmpty()
        //                           join mt1 in _context.MasterTable on new { Codigo = mq.Codtipomaquina, Master = "MQT" } equals new { Codigo = mt1.Codigo, Master = mt1.Master } into mt1Join
        //                           from mt1 in mt1Join.DefaultIfEmpty()
        //                           join mc in _context.Marca on new { mq.Marca, mq.Codtipomaquina } equals new { mc.Codmarca, mc.Codigotipomaq } into mcJoin
        //                           from mc in mcJoin.DefaultIfEmpty()
        //                           join md in _context.Modelo on new { mq.Modelo, mc.CodMarca, mc.CodigoTipoMaq } equals new { md.Codmodelo, md.Codmarca, md.Codigotipomaq } into mdJoin
        //                           from md in mdJoin.DefaultIfEmpty()
        //                           where asm.CodAgencia == cagencia && asm.Ccia == codcia
        //                           select new
        //                           {
        //                               mq.CodMaquina,
        //                               mq.NSerie,
        //                               TipoMaquina = mt1.Nombre,
        //                               Marca = mc.NombreMarca,
        //                               Modelo = md.NombreModelo
        //                           }).ToListAsync();

        //    if (!resultado.Any())
        //    {
        //        return NotFound("No se ha encontrado ninguna asignación de máquina para el cliente especificado.");
        //    }
