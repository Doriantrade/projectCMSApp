...
namespace CMS_System.Controllers
{
    ...
    public class CronogramaController : ControllerBase
    {
        ...
        public CronogramaController(cmsDb2024Context context, IHubContext<CronoAsignacionHUb> cronoAsignacionHUb)
        {
            _context = context;
        ...
        public async Task<IActionResult> GuardarCrono([FromBody] Cronograma model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        var result = await (
                            from cr in _context.Cronograma
                            ...
                            join mt1 in _context.MasterTable on new { codigo = ag.CodProv, master = "PRV00" } equals new { codigo = mt1.Codigo, master = mt1.Master } into mt1Join
                            from mt1 in mt1Join.DefaultIfEmpty()
                            join mt2 in _context.MasterTable on new { codigo = ag.CodCanton, master = ag.CodProv } equals new { codigo = mt2.Codigo, master = mt2.Master } into mt2Join
                            from mt2 in mt2Join.DefaultIfEmpty()
                            join img1 in _context.ImgFile on ag.Codcliente equals img1.Codentidad into img1Join
                            from img1 in img1Join.DefaultIfEmpty()
                            join mcro in _context.Mantemaqcro
